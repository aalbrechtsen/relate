     ...
     
{
  double f0;
  int i;
  double *space =calloc((2*npar+10),sizeof(double));
...
double findmax_bfgs(int numpars, double *invec, double (*fun)( double x[]),
  ...
  if (dfun==NULL) {
    ...
    lowbound0 = lowbound;
    upbound0 = upbound;
    need0 = calloc(numpars,sizeof(int));
    for (i=0; i<numpars; i++) need0[i]=1;
  }
  else dfun0 = dfun;
  if (nbd==NULL) {
    nbd0 = calloc(numpars,sizeof(int));
    for (i=0; i<numpars; i++) nbd0[i]=2;
  }
  else nbd0 = nbd;
  
  grad = calloc(numpars,sizeof(double));
  wa = calloc(((2*m+4)*numpars + 12*m*m + 12*m),sizeof(double));
  iwa = calloc(3*numpars,sizeof(int));
  memset(task,0,60);
  memset(csave,0,60);
